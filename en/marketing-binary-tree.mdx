...
When a user activates an NFT, they are permanently placed into a binary
marketing structure with a maximum depth of 22 levels — holding up to
...
<CardGroup cols={3}>
  ...
  <Card title="Spillover" icon="arrow-down">
    ...
    You earn full rewards from spillover — same as direct invites.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="You Earn from Depth">
    ...
    GENESIS earns from 2 levels. INFINITY earns from 19 levels.
  </Step>
</Steps>

## Tree Distribution Overview

Each level of the binary tree has a fixed number of positions and a corresponding reward percentage. Your NFT level determines how deep into this structure you can earn.

<Frame>
  <img src="/Marketing.png" alt="Binary tree distribution table — 22 levels with participant counts and reward percentages" style={{ borderRadius: '12px' }} />
</Frame>
...
<Info>
  **Sponsor must be an active tree node.** When a new user is registered through `registerUser()`, the smart contract does not accept any existing user ID as a sponsor — the supplied sponsor must be an **active node in the binary tree**. Concretely, the sponsor must:
