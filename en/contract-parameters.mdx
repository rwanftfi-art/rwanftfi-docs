...
<Note>
  **Loan Eligibility Threshold — semantics:** This is **not** a loan duration. It is the minimum time-to-next-auto-sell that a DA stack must still have remaining in order to be used as collateral for a **new** loan (on-chain check: `_getTimeToNextAutosale >= 30 days`). Once a loan is open, there is no fixed term — the borrower can call `repay()` at any time while the stack is still active, and `repay()` only reverts after the stack has entered its auto-sell cycle.
</Note>
