...
2. **Solde Cumulatif (`accumulativeBalance`) :** Un compte d'épargne obligatoire où 20 % de chaque récompense marketing sont crédités immédiatement à l'accrual.
   ...
   * **Redistribution à 120 jours :** Si l'utilisateur n'utilise pas son Solde Cumulatif dans les 120 jours, le solde inutilisé devient éligible à la redistribution. La répartition dépend du type de NFT de l'utilisateur :
   ...
   <Note>
     **Le destinataire d'un transfert doit être un nœud actif.** Un transfert de Solde Cumulatif vers un autre utilisateur ne réussit que si le **destinataire existe déjà en tant que nœud actif** dans le système. La création d'un compte (email/mot de passe) ne suffit pas à elle seule - le destinataire doit d'abord être entré dans le système de l'une des deux façons suivantes :

     - Avoir effectué **n'importe quel dépôt** sur son solde (même minimal), ou
     - **Avoir activé un NFT Cadeau**, ce qui l'enregistre en tant que nœud dans la structure marketing.

     Si le destinataire n'a fait ni l'un ni l'autre, le transfert ne peut pas aboutir : la **transaction de l'émetteur est revertée**, et l'opération reste bloquée jusqu'à ce que le destinataire active sa présence on-chain. Cela reflète la règle [**Le sponsor doit être un nœud actif de l'arbre**](/fr/marketing-binary-tree) - un mappage uniquement d'identité (un wallet auquel un ID utilisateur a été attribué mais sans état actif on-chain) ne peut pas participer, que ce soit comme sponsor ou comme destinataire d'un transfert de Solde Cumulatif.
   </Note>
