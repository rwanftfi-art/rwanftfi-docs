...
2. **Saldo Akumulatif (`accumulativeBalance`):** Akun tabungan wajib di mana 20% dari setiap imbalan marketing dikreditkan segera saat akrual.
   ...
   * **Redistribusi 120 Hari:** Jika pengguna tidak menggunakan Saldo Akumulatif mereka dalam 120 hari, saldo yang tidak terpakai memenuhi syarat untuk redistribusi. Pembagian tergantung pada jenis NFT pengguna:
   ...
   <Note>
     **Penerima transfer harus berupa node aktif.** Transfer Saldo Akumulatif ke pengguna lain hanya berhasil jika **penerima sudah ada sebagai node aktif** dalam sistem. Membuat akun (email/kata sandi) saja tidak cukup - penerima harus terlebih dahulu masuk ke dalam sistem melalui salah satu dari dua cara:

     - Melakukan **deposit apa pun** ke saldo mereka (bahkan yang minimal), atau
     - **Mengaktifkan NFT Hadiah**, yang mendaftarkan mereka sebagai node dalam struktur marketing.

     Jika penerima belum melakukan keduanya, transfer tidak dapat diselesaikan: **transaksi pengirim akan dibatalkan (revert)**, dan operasi tetap terblokir hingga penerima mengaktifkan kehadirannya secara on-chain. Ini mencerminkan aturan [**Sponsor harus berupa node pohon yang aktif**](/id/marketing-binary-tree) - pemetaan identitas saja (dompet yang memegang ID pengguna tetapi tidak memiliki status on-chain aktif) tidak dapat berpartisipasi, baik sebagai sponsor maupun sebagai penerima transfer Saldo Akumulatif.
   </Note>
