...
export const SellVsLend = () => {
  ...
  const getRecommendation = () => {
    if (daAmount <= 0 || daPrice <= 0) return { text: 'Enter DA amount and price to see recommendation.', color: 'text-[#A0A0A0]' };
  ...
  return (
    <div className="p-6 rounded-xl not-prose bg-[#0A0A0A] border border-white/5 dark:bg-white dark:border-zinc-200">
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-serif italic text-white dark:text-zinc-900">Sell vs Lend Decision Tool</h3>
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        <div className="flex-1 rounded-xl bg-[#141420] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
          <label className="text-[10px] font-semibold text-[#A0A0A0] uppercase tracking-wider mb-2 block">DA Amount</label>
          <input type="number" min="0" max="1000000" step="1" value={daAmount}
            onChange={(e) => setDaAmount(Math.max(0, Number(e.target.value)))}
            className="w-full p-3 rounded-lg bg-[#1C1C2E] border border-white/5 text-white text-lg font-bold dark:bg-zinc-100 dark:border-zinc-200 dark:text-zinc-900"
            placeholder="1000" />
        </div>
        <div className="flex-1 rounded-xl bg-[#141420] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
          <label className="text-[10px] font-semibold text-[#A0A0A0] uppercase tracking-wider mb-2 block">Current DA Price (USDT)</label>
        ...
        <div className="flex flex-col gap-3 md:w-56 flex-shrink-0">
          <div className="flex bg-[#1C1C2E] dark:bg-zinc-100 rounded-full p-1">
            <button onClick={() => setSellMode('manual')}
              className={`flex-1 px-3 py-1.5 rounded-full text-[11px] font-medium transition-all ${sellMode === 'manual' ? 'bg-red-500 text-white' : 'text-[#A0A0A0]'}`}>
              Manual (75%)
            </button>
            <button onClick={() => setSellMode('auto')}
              className={`flex-1 px-3 py-1.5 rounded-full text-[11px] font-medium transition-all ${sellMode === 'auto' ? 'bg-red-500 text-white' : 'text-[#A0A0A0]'}`}>
              Auto (70%)
            </button>
          </div>
          <div className="rounded-xl bg-[#141420] border border-white/5 p-3 text-center dark:bg-zinc-50 dark:border-zinc-200">
            <div className="text-[10px] text-[#A0A0A0] uppercase">Position Value</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="rounded-xl border-2 border-green-500/20 bg-green-500/5 p-5 dark:bg-green-50 dark:border-green-200">
          ...
          {sellMode === 'auto' && (
            <div className="mt-2 px-3 py-2 rounded-lg border border-[#2563EB]/20 bg-[#2563EB]/5">
      ...
      {/* Summary Bar */}
      <div className="rounded-xl bg-[#141420] border border-white/5 p-5 dark:bg-zinc-50 dark:border-zinc-200">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            <div className="text-[10px] font-semibold text-[#A0A0A0] uppercase tracking-wider mb-2">Comparison Summary</div>
            <div className="flex items-baseline gap-3 mb-2 flex-wrap">
              ...
              <span className="text-sm font-semibold text-[#A0A0A0]">Δ {fmtUsd(Math.abs(sellVsLendDiff))} ({diffPct.toFixed(1)}%)</span>
            </div>
            <p className={`text-sm font-medium ${rec.color}`}>{rec.text}</p>
          </div>
          {/* Visual bar */}
          <div className="w-full sm:w-48 flex-shrink-0">
            <div className="text-[10px] text-[#A0A0A0] mb-1 text-center">Payout comparison</div>
            <div className="flex rounded-full overflow-hidden h-4 bg-[#1C1C2E] dark:bg-zinc-200">
              {(() => {
            ...
            <div className="flex justify-between text-[9px] text-[#A0A0A0] mt-0.5"><span>Sell</span><span>Lend</span></div>
          </div>
        </div>
      </div>

      {/* Disclaimer */}
      <div className="text-xs text-[#A0A0A0] leading-relaxed space-y-1 mt-4 dark:text-zinc-500">
        <p>Simplified model — assumes initial supply of 21M DA. Actual values depend on ecosystem activity.</p>
        <p>
          Manual sell: <code className="px-1 bg-[#1C1C2E] dark:bg-zinc-200 rounded text-white dark:text-zinc-900">payout = amount × price × 0.75</code> (burn 25%).{' '}
          Auto-sell: <code className="px-1 bg-[#1C1C2E] dark:bg-zinc-200 rounded text-white dark:text-zinc-900">payout = amount × price × 0.70</code> (burn 30%).{' '}
          Lend: <code className="px-1 bg-[#1C1C2E] dark:bg-zinc-200 rounded text-white dark:text-zinc-900">loan = amount × price × 0.70</code>. Loan cutoff: 30 days.
        </p>
      </div>
    </div>
  );
