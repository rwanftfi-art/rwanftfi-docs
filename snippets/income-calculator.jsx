...
export const IncomeCalculator = () => {
  ...
  const presets = {
    small: () => {
      const newLevels = levels.map((l) => ({ ...l, sales: 0, rawSales: '', avgPrice: 275 }));
      newLevels[1] = { ...newLevels[1], sales: 2, rawSales: '2' };
      newLevels[2] = { ...newLevels[2], sales: 2, rawSales: '2' };
      newLevels[3] = { ...newLevels[3], sales: 2, rawSales: '2' };
      return newLevels;
    },
    growing: () => {
      const newLevels = levels.map((l) => ({ ...l, sales: 0, rawSales: '', avgPrice: 550 }));
      for (let i = 1; i <= 8; i++) {
        const s = Math.min(3 + i, MAX_PARTICIPANTS[i]);
        newLevels[i] = { ...newLevels[i], sales: s, rawSales: String(s) };
      }
      return newLevels;
    },
    clear: () => levels.map((l) => ({ ...l, sales: 0, rawSales: '' })),
  };
                    ...
                    {isLocked ? (
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateLevel(i, { sales: num, rawSales: val });
                        }}
