  ...
  return (
    <div className="p-6 rounded-xl not-prose bg-[#0A0A0A] border border-white/5 dark:bg-white dark:border-zinc-200">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-serif italic text-white dark:text-zinc-900">Marketing Income Calculator</h3>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="md:w-72 flex-shrink-0 space-y-4">
          <div className="rounded-xl bg-[#141420] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
            <label className="text-[10px] font-semibold text-[#A0A0A0] uppercase tracking-wider mb-2 block">Your NFT Level</label>
            <select value={nftLevel} onChange={(e) => setNftLevel(Number(e.target.value))}
              className="w-full p-2.5 rounded-lg bg-[#1C1C2E] border border-white/5 text-white text-sm font-medium dark:bg-zinc-100 dark:border-zinc-200 dark:text-zinc-900">
          ...
          <div className="rounded-xl bg-[#141420] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs text-[#A0A0A0]">Avg NFT Sale in Team</label>
            ...
            <div className="flex justify-between text-[10px] text-[#A0A0A0] mt-1"><span>$28</span><span>$24,000</span></div>
          </div>

          <div className="rounded-xl bg-[#141420] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
            <div className="flex justify-between items-center mb-2">
              <label className="text-xs text-[#A0A0A0]">Sales per Level</label>
            ...
            <input type="range" min="1" max="50" step="1" value={salesPerLevel}
              ...
              className="w-full h-2 bg-[#1C1C2E] rounded-lg appearance-none cursor-pointer accent-[#2563EB] dark:bg-zinc-200"
              style={{ touchAction: 'manipulation' }} />
            <div className="flex justify-between text-[10px] text-[#A0A0A0] mt-1"><span>1</span><span>50</span></div>
          </div>

          <div className="rounded-xl p-5 text-center" style={{ background: 'linear-gradient(135deg, #2563EB, #4040FF)' }}>
            <div className="text-xs uppercase tracking-wider text-white/70 mb-1">Total Gross Income</div>
            <div className="text-2xl md:text-3xl font-black text-white transition-all duration-300">{fmtUsd(totalIncome)}</div>
            <div className="text-xs text-white/60 mt-1">{maxDepth} active levels × {salesPerLevel} sales</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 space-y-4">
          <div className="rounded-xl bg-[#141420] border border-white/5 p-4 overflow-x-auto dark:bg-zinc-50 dark:border-zinc-200">
            <div className="text-xs font-semibold text-[#A0A0A0] mb-2">Income by Marketing Level (22 levels)</div>
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: '500px' }}>
              <defs><linearGradient id="incBarGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2563EB" /><stop offset="100%" stopColor="#4040FF" /></linearGradient></defs>
              {[0.25, 0.5, 0.75, 1].map(frac => (
                <g key={frac}>
                  ...
                  <text x={PAD.left - 6} y={PAD.top + cH - cH * frac} textAnchor="end" dominantBaseline="middle" fill="#A0A0A0" style={{ fontSize: '8px' }}>{fmtUsd(maxBar * frac)}</text>
                </g>
              ))}
              {levelIncome.map((l, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    <text x={bx + barW / 2} y={PAD.top + cH + 12} textAnchor="middle" fill="#A0A0A0" style={{ fontSize: '7px' }}>{l.level}</text>
                    <text x={bx + barW / 2} y={PAD.top + cH + 22} textAnchor="middle" fill="rgba(160,160,160,0.5)" style={{ fontSize: '6px' }}>{l.pct}%</text>
                    {!l.isUnlocked && !l.isPhase2Only && <text x={bx + barW / 2} y={PAD.top + cH - 6} textAnchor="middle" style={{ fontSize: '7px' }}>🔒</text>}
                    {l.isPhase2Only && <text x={bx + barW / 2} y={PAD.top + cH - 6} textAnchor="middle" fill="rgba(160,160,160,0.4)" style={{ fontSize: '5.5px', fontWeight: 'bold' }}>P2</text>}
              ...
              <text x={W / 2} y={H - 2} textAnchor="middle" fill="#A0A0A0" style={{ fontSize: '8px' }}>Marketing Level →</text>
            </svg>
            <div className="flex gap-4 mt-2 justify-center flex-wrap">
              <span className="flex items-center gap-1.5 text-[10px] text-[#A0A0A0]"><span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: 'linear-gradient(135deg, #2563EB, #4040FF)' }} /> Active</span>
              <span className="flex items-center gap-1.5 text-[10px] text-[#A0A0A0]"><span className="w-2.5 h-2.5 rounded-sm bg-white/5 inline-block" /> 🔒 Locked</span>
              <span className="flex items-center gap-1.5 text-[10px] text-[#A0A0A0]"><span className="w-2.5 h-2.5 rounded-sm bg-white/[0.03] inline-block" /> Phase 2</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="rounded-xl bg-[#2563EB]/5 border border-[#2563EB]/20 p-4 dark:bg-blue-50 dark:border-blue-200">
              <div className="text-[10px] uppercase tracking-wider text-[#2563EB] font-semibold mb-1">5% → DA Liquidity</div>
              <div className="text-xl font-black text-[#2563EB] transition-all duration-300">{fmtUsd(daTax)}</div>
              <div className="text-[11px] text-[#2563EB]/60 mt-1">Feeds into DA pool → increases DA price</div>
            </div>
          </div>

          {maxDepth < 22 && (
            <div className="px-4 py-3 rounded-lg bg-[#1C1C2E] border border-white/5 dark:bg-zinc-50 dark:border-zinc-200">
              <div className="text-xs text-[#A0A0A0] dark:text-zinc-500">
