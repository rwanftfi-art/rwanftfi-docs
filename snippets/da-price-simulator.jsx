...
const PRESETS = [
  ...
  { label: '6 Months',  burned: 2_100_000, liqBonus: 5_000_000,  price: null },
...
export const DaPriceSimulator = () => {
  ...
  const miningCalc = useMemo(() => {
    ...
    return { cycle1Nftm, cycle2Nftm, totalNftm, daReceived, holdValue, sellPayout, lendPayout, pipelineDays, getValue, getRoi };
  }, [nft, daPrice]);
  ...
  const curveData = useMemo(() => {
    ...
    for (let i = 0; i <= N; i++) {
      ...
      pts.push({ i, price: calcPrice(liq, supply) });
    }
    return pts;
  ...
  const allNfts = useMemo(() => {
    return Object.entries(NFT_DATA).map(([lvl, d]) => {
      const totalNftm = d.price * d.c1 + d.price * d.c2;
      const da = totalNftm / daPrice;
      const sellReturn = da * daPrice * 0.75;
      const roi = ((sellReturn - d.price) / d.price) * 100;
      const pipeline = d.days + d.days + 3;
      return { level: Number(lvl), ...d, totalNftm, da, sellReturn, roi, pipeline };
    });
  }, [daPrice]);
          ...
          <div style={{ backgroundColor: '#383838', borderRadius: '12px', padding: '24px' }}>
            ...
            <div className="flex flex-col md:flex-row" style={{ gap: '16px', marginBottom: '24px' }}>
              <div style={{ flex: 1 }}>
                ...
                >
                  {Object.entries(NFT_DATA).map(([lvl, d]) => (
                    <option key={lvl} value={lvl}>L{lvl} — {d.name} ({fmtUsd(d.price)})</option>
                  ))}
                </select>
              </div>
              <div style={{ flex: 1 }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              ...
              <div className="flex items-center justify-center" style={{ gap: '12px', marginTop: '12px' }}>
                ...
                <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>~{miningCalc.pipelineDays} days</span>
              </div>
            </div>
          ...
          <div style={{ backgroundColor: '#383838', borderRadius: '12px', padding: '24px' }}>
            ...
            <div className="hidden md:block">
              <svg viewBox="0 0 700 90" style={{ width: '100%' }}>
                ...
                <text x="640" y="72" textAnchor="middle" fill="rgba(255,255,255,0.4)" style={{ fontSize: '10px' }}>{fmtNum(miningCalc.daReceived)} DA</text>
              </svg>
            </div>
            ...
            <div className="md:hidden" style={{ display: 'flex', flexDirection: 'column', gap: '0', paddingLeft: '20px' }}>
              {[
                ...
                { label: `Mining (${nft.days} days)`, sub: `${fmtNum(miningCalc.totalNftm)} NFTM`, isAlert: false },
                { label: '72h Lock', sub: '⚠️ Mandatory wait', isAlert: true },
                { label: `Farm (${nft.days} days)`, sub: `${fmtNum(miningCalc.daReceived)} DA`, isAlert: false },
              ].map((step, i) => (
