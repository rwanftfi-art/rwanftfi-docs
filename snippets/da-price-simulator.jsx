      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="rounded-xl bg-[#383838] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium text-white/40">Tokens Burned</label>
          ...
          <div className="flex justify-between text-[10px] text-white/40 mt-1"><span>0</span><span>20M DA</span></div>
        </div>
        <div className="rounded-xl bg-[#383838] border border-white/5 p-4 dark:bg-zinc-50 dark:border-zinc-200">
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium text-white/40">Liquidity Inflow</label>
          ...
          <input type="range" min="0" max="50000000" step="500000" value={liquidityInflow}
            ...
            className="w-full h-2 bg-[#383838]/70 rounded-lg appearance-none cursor-pointer accent-white dark:bg-zinc-200"
            style={{ touchAction: 'manipulation' }} />
          <div className="flex justify-between text-[10px] text-white/40 mt-1"><span>$0</span><span>$50M</span></div>
        </div>
      </div>

      {/* SVG Chart */}
      <div className="rounded-xl bg-[#383838] border border-white/5 p-4 overflow-x-auto dark:bg-zinc-50 dark:border-zinc-200">
        <div className="text-xs font-semibold text-white/40 mb-2">Price Curve</div>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: '400px' }}>
          ...
          {yVals.map((val, i) => (
            <g key={i}>
              ...
              <text x={PAD.left - 8} y={getY(val)} textAnchor="end" dominantBaseline="middle" fill="rgba(255,255,255,0.4)" style={{ fontSize: '9px' }}>${val.toFixed(2)}</text>
            </g>
          ))}
          {/* Area */}
